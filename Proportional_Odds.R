##########################################################
####Example file that fits a simple cumulative logit model
####This is what the initial paper is based off
##########################################################
library(MASS)
DATA<-read.csv("Data for Bioinformatics - no Internationalt2.csv")
####Prep the data
races<-c("AFA","API","CAU","HIS","UNK")
m=1:5
DATA=DATA[which(DATA$Race %in% races[m]),]
